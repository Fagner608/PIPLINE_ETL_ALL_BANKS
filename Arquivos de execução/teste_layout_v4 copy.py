...
def main():
    ...
    if st.button('Filtrar Contratos'):
        ...
        if not contratos_filtrados.empty:
            ...
            num_cols = contratos_filtrados[['valor_liquido', 'valor_base', 'valor_cms_repasse']].columns
            
            if not num_cols.empty:
                # contratos_filtrados[num_cols] = contratos_filtrados[num_cols].map(lambda x: float(x.replace(".", "").replace(",", ".")))
                for col in num_cols:
                    contratos_filtrados[col] = contratos_filtrados[col].map(lambda x: float(x.replace(".", "").replace(",", ".")) if isinstance(x, str) else x)
                stats = contratos_filtrados[num_cols].describe()
                st.dataframe(stats)

            # Contagem total de registros
            st.write(f"**Total de Registros:** {contratos_filtrados.shape[0]}")

            # Estatísticas categóricas
            st.write("### Estatísticas Categóricas")
            cat_cols = contratos_filtrados[['tipo_contrato', 'status_importacao']].columns
            for col in cat_cols:
                st.write(f"**{col}**")
                st.write(contratos_filtrados[col].value_counts())
