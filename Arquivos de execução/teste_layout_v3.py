...
def main():
    ...
    with st.sidebar:
        ...
        nome_banco = st.selectbox('Banco', options=[x[0] for x in nome_banco_options])

        status_importacao_options = cursor.execute("SELECT DISTINCT status_importacao FROM contrato").fetchall()
        status_importacao = st.selectbox('Status de Importação', options=[x[0] for x in status_importacao_options])

        tipo_operacao_options = cursor.execute("SELECT DISTINCT tipo_operacao FROM contrato").fetchall()
        tipo_operacao = st.selectbox('Tipo de Operação', options=[x[0] for x in tipo_operacao_options])
